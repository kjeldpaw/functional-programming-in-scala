...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    def mostRetweeted(tweetSet: TweetSet): Tweet = {
      lazy val mostRetweeted: Tweet = tweetSet.mostRetweeted
      if(mostRetweeted.retweets > elem.retweets) {
        mostRetweeted
      } else {
        elem
      }
    ...
    if(!left.isEmpty && !right.isEmpty) {
      lazy val leftMostRetweeted: Tweet = mostRetweeted(left)
      lazy val rightMostRetweeted: Tweet = mostRetweeted(right)
